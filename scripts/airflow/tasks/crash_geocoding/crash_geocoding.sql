...
CREATE OR REPLACE FUNCTION "TRAFFIC".get_intersection(collision_lat NUMERIC, collision_long NUMERIC, tbl_name TEXT)
...
format('SELECT gid
	...
	ORDER BY st_transform(geom, 98012) <-> ST_Transform(ST_SetSRID(ST_MakePoint(%s, %s), 4326), 98012)
	LIMIT 1'
	, 
...
CREATE TABLE "TRAFFIC".collision_events_geocoded AS (
SELECT collision_accnb, collision_date, collision_time, day_no, px, street_1, street_type_1, direction_1, street_2, street_type_2, direction_2,
    street_3, street_type_3, direction_3, location_class, location_desc, collision_type, impact_type, road_class, visibility, light, 
    road_surface_cond, longitude, latitude, output_gid_arr[1] AS gid, output_gid_arr[2] AS tbl_type, 	
	...
	SELECT *, 
			...
			FROM 
			(
			SELECT DISTINCT ON (collision_accnb, collision_date) 
				*,
				"TRAFFIC".get_intersection(latitude::NUMERIC, longitude::NUMERIC, 'gis.centreline_intersection'::TEXT) AS geom_int
			FROM "TRAFFIC".collision_events
