...
CREATE MATERIALIZED VIEW IF NOT EXISTS prj_volume.artery_centreline AS (
  ...
  FROM artery_centreline_raw
  GROUP BY arterycode, centreline_id, centreline_type
);
create unique index IF NOT EXISTS artery_centreline_arterycode on prj_volume.artery_centreline (arterycode);
create index IF NOT EXISTS artery_centreline_centreline on prj_volume.artery_centreline (centreline_type, centreline_id);
