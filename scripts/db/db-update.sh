...
DIR_DB="$GIT_SCRIPTS/db"

# install schema if necessary
if psql -U flashcrow -tAc "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'APP_META' AND table_name = 'DB_UPDATE')" | grep f; then
  echo "Installing db-update metadata tables..."
  DB_UPDATE_SQL_FILE="$DIR_DB/db-update-install.sql"
  psql -U flashcrow -f "$DB_UPDATE_SQL_FILE"
  if psql -U flashcrow -tAc "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'APP_META' AND table_name = 'DB_UPDATE')" | grep f; then
...
CURRENT_VERSION=$(psql -U flashcrow -tAc 'SELECT \"currentVersion\" from \"APP_META\".\"DB_UPDATE\"')
...
if [ "$TARGET_VERSION" -gt "$CURRENT_VERSION" ]; then
  ...
  for i in $(seq $CURRENT_VERSION -1 $((TARGET_VERSION+1))); do
    ...
    psql -U flashcrow "$DOWN_SQL_FILE"
  done
