...
grep -E "^5" features/counts.txt | sed -r 's/^(.*)\t(.*)$/\/reporter\/reports?type=WARRANT_TRAFFIC_SIGNAL_CONTROL\&id=\1%2F\2\&format=PDF\&adequateTrial=true\&preparedBy=test\&preventablesByYear=2\&preventablesByYear=3\&preventablesByYear=1\&startYear=1999/' > paths/getReport_WARRANT_TRAFFIC_SIGNAL_CONTROL.txt
grep -E "^5" features/counts.txt | sed -r 's/^(.*)\t(.*)$/\/reporter\/reports?type=WARRANT_TRAFFIC_SIGNAL_CONTROL\&id=\1%2F\2\&format=WEB\&adequateTrial=true\&preparedBy=test\&preventablesByYear=2\&preventablesByYear=3\&preventablesByYear=1\&startYear=1999/' > paths/getReportWeb_WARRANT_TRAFFIC_SIGNAL_CONTROL.txt
