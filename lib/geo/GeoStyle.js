...
function normalizeAndSeparateLayers(style) {
  ...
  /* eslint-disable-next-line no-param-reassign */
  style.sprite = `${origin}/icons/map2/sprite`;
...
function injectSources(style) {
  ...
  addDynamicTileSource(style, 'studies:1', MapZoom.LEVEL_2, MapZoom.LEVEL_1);
...
class GeoStyle {
  ...
  static getStudiesLayers({ datesFrom, layers: { studies } }) {
    const source = `studies:${datesFrom}`;
    ...
    return [
      {
        ...
        layout: {
          ...
          'icon-image': [
            'case',
            ['get', 'multi'], 'location-multi',
            'location-single',
          ],
          'text-allow-overlap': true,
          'text-field': [
            'case',
            ['get', 'multi'], ['to-string', ['get', 'locationIndex']],
            '',
          ],
          'text-font': [
            'Roboto Medium',
