...
class GeoStyle {
  ...
  static getCollisionsFilter({ filtersCollision, filtersCommon }) {
    ...
    if (hoursOfDayStart !== 0 || hoursOfDayEnd !== 24) {
      filters.push(
        ['>=', ['get', 'hourOfDay'], hoursOfDayStart],
        ['<', ['get', 'hourOfDay'], hoursOfDayEnd],
      );
    }
  ...
  static getCollisionsTimeFactorDateRange(applyDateRange, dateRangeStart, dateRangeEnd) {
    if (!applyDateRange) {
      return 10;
    }
    const nowMinus10Years = NOW.minus({ years: 10 });
    const a = { start: nowMinus10Years, end: NOW };
    const b = { start: dateRangeStart, end: dateRangeEnd };
  ...
  static getCollisionsTimeFactorHoursOfDay(hoursOfDayStart, hoursOfDayEnd) {
    const hours = hoursOfDayEnd - hoursOfDayStart;
    if (hours === 0) {
      return 1;
    }
    return hours / 24;
  ...
  static getCollisionsTimeFactor({ filtersCollision, filtersCommon }) {
    const {
      applyDateRange,
      dateRangeStart,
      dateRangeEnd,
    ...
    let timeFactor = GeoStyle.getCollisionsTimeFactorDateRange(
      applyDateRange,
      dateRangeStart,
      dateRangeEnd,
    );
    if (daysOfWeek.length > 0) {
      timeFactor *= daysOfWeek.length / 7;
    }
    timeFactor *= GeoStyle.getCollisionsTimeFactorHoursOfDay(hoursOfDayStart, hoursOfDayEnd);
    return timeFactor;
  }
