...
const [LAYERS_NON_SYMBOL_AERIAL] = normalizeAndSeparateLayers(STYLE_BASE_MAP_LIGHT);
...
class GeoStyle {
    ...
    return [
      {
        ...
        paint: {
          ...
          'heatmap-intensity': [
            ...
            ['linear'],
            ['zoom'],
            MapZoom.LEVEL_3.minzoom, 1,
            MapZoom.LEVEL_3.maxzoomSource + 1, 3,
          ],
          'heatmap-opacity': [
            'interpolate',
            ['linear'],
  ...
  static getCountsLayers({ layers: { counts } }, baseStyle) {
    ...
    return [
      getLayer(baseStyle, 'counts', 'symbol', {
        layout: {
          'icon-image': 'count',
          'text-field': [
            'case',
