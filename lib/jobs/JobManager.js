...
class JobManager {
  ...
  static async fetch() {
    const jobs = await PG_BOSS.fetch('*', 1, { includeMetadata: true });
    if (jobs === null) {
      return [];
    }
  ...
  static async run(job) {
    ...
    try {
      ...
      await PG_BOSS.complete(job.id);
      jobMetadata.state = 'completed';
    } catch (err) {
      await PG_BOSS.fail(job.id);
      jobMetadata.state = 'failed';
    }
    await JobMetadataDAO.update(jobMetadata);
  }
}
