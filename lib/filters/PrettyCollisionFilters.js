...
async function generateFilterFile(filters, location) {
  const freshFilters = await generateCollisionFilters(filters);
  const formattedFilters = formatFilters(freshFilters);
  return ['Collision Directory Report', location, '', 'The attached CSV file contains collision records that match the following criteria:', '', formattedFilters];
}
