...
const dbConnection = process.env.LOGNAME === 'vagrant' ? 'postgres://flashcrow@localhost:5432/flashcrow' : 'postgres://flashcrow:flashcrow@flashcrow:5432/flashcrow';
 ...
 * separately for dev, QA, staging, and production.  Note that this is a different usage of
 * "production" from that used by City of Toronto Technology Services, which uses this to refer
 * to environments in which fully audited and approved applications are run.
 */
...
const configSchema = Joi.object().keys({
  ...
  /*
   * `sendGrid` is deprecated; do not add it to new environments!  It is included here for
   * backwards compatibility with earlier private config versions.  We no longer use SendGrid
   * in any of our environments, whether local or on EC2, and the associated dependencies
   * have been removed.
   */
