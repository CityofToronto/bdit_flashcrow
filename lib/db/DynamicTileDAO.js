...
FROM schools`;
  ...
  static async getStudiesFeatures(tileInfo, datesFrom) {
    const datesFromYear = `${datesFrom} year`;
    const sql = `
WITH studies AS (
  SELECT
  ...
  WHERE ST_Intersects(
    ST_Transform(geom, 3857),
    ST_MakeEnvelope($(xmin), $(ymin), $(xmax), $(ymax), 3857)
  ) AND start_date >= now() - interval $(datesFromYear)
...
FROM studies`;
  ...
  static async getTileFeatures(layerName, z, x, y) {
    ...
    if (layerName.startsWith('studies')) {
      const [, datesFromStr] = layerName.split(':');
      const datesFrom = parseInt(datesFromStr, 10);
      if (Number.isNaN(datesFrom)) {
        throw new InvalidDynamicTileLayerError(layerName);
      }
      return DynamicTileDAO.getStudiesFeatures(tileInfo, datesFrom);
    }
    throw new InvalidDynamicTileLayerError(layerName);
  }
}
