...
FROM collisions`;
  ...
  static async getCountsFeatures(tileInfo, datesFrom) {
    const datesFromYear = `${datesFrom} year`;
    const sql = `
WITH counts AS ((
  SELECT ac.centreline_id, ac.centreline_type,
  MODE() WITHIN GROUP(ORDER BY ac.geom) AS geom,
  ...
  WHERE ST_Intersects(
    ...
    ST_MakeEnvelope($(xmin), $(ymin), $(xmax), $(ymax), 3857)
  ) AND ci."COUNT_DATE" >= now() - interval $(datesFromYear)
  GROUP BY ac.centreline_id, ac.centreline_type
) UNION ALL (
  SELECT ac.centreline_id, ac.centreline_type,
  MODE() WITHIN GROUP(ORDER BY ac.geom) AS geom,
  ...
  WHERE ST_Intersects(
    ST_Transform(ac.geom, 3857),
    ST_MakeEnvelope($(xmin), $(ymin), $(xmax), $(ymax), 3857)
  ) AND cim."COUNT_DATE" >= now() - interval $(datesFromYear)
  GROUP BY ac.centreline_id, ac.centreline_type
))
SELECT ST_AsGeoJSON(
  ST_SnapToGrid(
...
FROM schools`;
  ...
  static async getTileFeatures(layerName, z, x, y) {
    ...
    if (layerName.startsWith('collisionsLevel1')) {
      ...
      if (Number.isNaN(datesFrom)) {
        throw new InvalidDynamicTileLayerError(layerName);
      }
      return DynamicTileDAO.getCollisionsFeatures(tileInfo, datesFrom);
    }
    if (layerName.startsWith('counts')) {
      const [, datesFromStr] = layerName.split(':');
      const datesFrom = parseInt(datesFromStr, 10);
      if (Number.isNaN(datesFrom)) {
        throw new InvalidDynamicTileLayerError(layerName);
      }
      return DynamicTileDAO.getCountsFeatures(tileInfo, datesFrom);
    }
