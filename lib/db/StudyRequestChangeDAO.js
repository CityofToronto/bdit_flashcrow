...
INSERT INTO "study_request_changes" (
   ...
   * @returns {Array<Object>} array of changes objects
   */
...
ORDER BY "createdAt" DESC`;
  ...
  /**
   * Fetch all changes for the given bulk study request.
   *
   * @param {Object} studyRequestBulk - bulk study request to fetch changes for
   * @returns {Array<Object>} array of changes objects
   */
  static async byStudyRequestBulk(studyRequestBulk) {
    const studyRequestIds = studyRequestBulk.studyRequests.map(({ id }) => id);
    const sql = `
SELECT * FROM "study_request_changes"
WHERE "studyRequestId" IN ($(studyRequestIds:csv))
ORDER BY "studyRequestId" ASC, "createdAt" DESC`;
    const changes = await db.manyOrNone(sql, { studyRequestIds });
    const studyRequestChangesSchema = Joi.array().items(StudyRequestChange.read);
    return studyRequestChangesSchema.validateAsync(changes);
  }
...
WHERE "studyRequestId" = $(studyRequestId)`;
  ...
  static async deleteByStudyRequests(studyRequests) {
    const studyRequestIds = studyRequests.map(({ id }) => id);
    const sql = `
DELETE FROM "study_request_changes"
WHERE "studyRequestId" IN ($(studyRequestIds:csv))`;
    const rowsDeleted = await db.result(sql, { studyRequestIds }, r => r.rowCount);
    return rowsDeleted > 0;
  }
}
