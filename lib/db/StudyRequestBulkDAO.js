...
ORDER BY "id" ASC`;
    ...
    const studyRequestsPendingByBulk = new Map();
    studyRequestsPending.forEach((studyRequest) => {
      const { studyRequestBulkId } = studyRequest;
      const studyRequestsPendingForBulk = setdefault(
        studyRequestsPendingByBulk,
        studyRequestBulkId,
        [],
      );
      studyRequestsPendingForBulk.push(studyRequest);
    });

    const studyRequestsBulkPending = [];
    studyRequestsBulk.forEach((studyRequestBulk) => {
      if (!studyRequestsPendingByBulk.has(studyRequestBulk.id)) {
        return;
      }
      const studyRequests = studyRequestsPendingByBulk.get(studyRequestBulk.id);
      studyRequestsBulkPending.push({
        ...studyRequestBulk,
        studyRequests,
      });
    });
    const studyRequestsBulkSchema = Joi.array().items(StudyRequestBulk.read);
    return studyRequestsBulkSchema.validateAsync(studyRequestsBulkPending);
  }
