...
async function validateStudies(studies) {
  ...
  const filteredAndNormalizedStudies = studiesNormalized
    .filter(element => !StudyType.enumValueOf(element.studyType).maskStudy);
  const studiesSchema = Joi.array().items(Study.read);
  return studiesSchema.validateAsync(filteredAndNormalizedStudies);
}
