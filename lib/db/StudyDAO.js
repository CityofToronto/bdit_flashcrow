...
class StudyDAO {
  ...
  static async byCentrelinePending(centrelineId, centrelineType) {
    const sql = `
SELECT
  s.*
  FROM "studies" s
...
UPDATE "studies" SET
  ...
  "notes" = $(notes)
WHERE "id" = $(id)`;
    await db.query(sql, study);
    return study;
  }

  /**
   * Updates studies for the given study request to match `studyRequest.studies`.
   * To update this list, `studyRequest.studies` is compared against the existing
   * set of studies returned by `StudyDAO.byStudyRequests`.
   *
   * TODO: batch CUD queries
   *
   * @param {Object} studyRequest - study request to update studies for
   * @returns {Promise<Array>} - updated list of studies
   */
  static async updateByStudyRequest(studyRequest) {
    ...
    // create any studies in `studies` but not in `currentStudies`
    const toCreate = studies
      .map((study, i) => [study, i])
      .filter(([{ id = null }]) => id === null || !currentStudyIds.has(id));

    // update any studies in both
    const toUpdate = studies.filter(({ id = null }) => id !== null && currentStudyIds.has(id));

    // delete any studies in `currentStudies` but not in `studies`
