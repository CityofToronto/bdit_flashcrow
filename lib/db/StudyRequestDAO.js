...
ORDER BY "studyRequestBulkId" ASC, "id" ASC`;
   ...
   * if it is not COMPLETED or CANCELLED
   *
   * @param {Array<Object>} features - centreline features
  ...
  static async byCentrelinePending(features) {
    ...
    const centrelineFilter = getCentrelineFilter(features);
    const pendingStatuses = ['REQUESTED', 'ASSIGNED', 'CHANGES_NEEDED', 'REJECTED'];
    const sql = `
SELECT ${STUDY_REQUESTS_FIELDS}
WHERE ${centrelineFilter}
AND "status" IN ($(pendingStatuses:csv))`;
    const studyRequests = await db.manyOrNone(sql, { pendingStatuses });
    const studyRequestsSchema = Joi.array().items(StudyRequest.read);
    return studyRequestsSchema.validateAsync(studyRequests);
  }
...
INSERT INTO "study_requests" (
  ...
  ST_SetSRID(ST_GeomFromGeoJSON($(geom)), 4326)
    ...
    await StudyRequestItemDAO.upsertByStudyRequestIds([persistedStudyRequest.id]);
...
UPDATE "study_requests" SET
  ...
  WHERE "id" = $(id)`;
    ...
    await StudyRequestItemDAO.upsertByStudyRequestIds([studyRequest.id]);
    if (studyRequest.studyRequestBulkId !== null) {
      await StudyRequestItemDAO.upsertByStudyRequestBulkIds([studyRequest.studyRequestBulkId]);
    }
  ...
  static async delete(studyRequest) {
    ...
    await StudyRequestItemDAO.deleteByStudyRequestIds([studyRequest.id]);
