...
class EmailStudyRequestConfirmation extends EmailBase {
  ...
  static renderHours({ automatic, label }, study) {
    ...
    if (automatic) {
      ...
      return `${label}: ${duration} hour count, on ${daysOfWeekHuman}`;
    }
    const { hours: hoursUpper } = study;
    const hours = hoursUpper.toLowerCase();
    return `${label}: ${hours} hours, on ${daysOfWeekHuman}`;
  }
...
module.exports = EmailStudyRequestConfirmation;
