...
function generateStudyTypeOther(studyType) {
  if (studyType === StudyType.OTHER) {
    return Random.choice(STUDY_TYPES_OTHER);
  }
  return null;
}
...
function generateStudyType() {
  ...
  const duration = studyType.isMultiDay ? 72 : null;
  const hours = studyType.isMultiDay ? null : Random.choice(StudyHours.enumValues);
