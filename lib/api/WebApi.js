...
async function getSpecialCaseCentrelineIds() {
  const specialCaseCentrelineIds = await apiClient.fetch('/hacky-cases');
  return specialCaseCentrelineIds;
}

async function getCautionCaseCentrelineIds() {
  const cautionCaseCentrelineIds = await apiClient.fetch('/caution-cases');
  return cautionCaseCentrelineIds;
}
...
async function setBannerMessage(auth, banner) {
  const { user, csrf } = auth;
  const url = '/users/banner';
  const options = {
    method: 'PUT',
    csrf,
  ...
  await apiClient.fetch(url, options);
  return true;
}
