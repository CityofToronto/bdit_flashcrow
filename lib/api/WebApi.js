...
async function getBulkMvcr(filename) {
  const options = {
    method: 'GET',
    responseType: 'blob',
  };
  return apiClient.fetch(`/bulk-mvcr/${filename}`, options);
}
...
async function postJobCompressMvcrs(csrf, mvcrs) {
  const options = {
    method: 'POST',
    csrf,
    data: JSON.stringify(mvcrs),
  };
...
async function postStudyRequestBulkCopy(csrf, studyRequestBulk) {
  const { id: studyRequestBulkId } = studyRequestBulk;
  const url = `/requests/study/bulk/${studyRequestBulkId}/copy`;
  const options = {
    method: 'POST',
    csrf,
  };
...
async function postStudyRequestCopy(csrf, studyRequest) {
  const { id: studyRequestId } = studyRequest;
  const url = `/requests/study/${studyRequestId}/copy`;
  const options = {
    method: 'POST',
    csrf,
  };
  const persistedStudyRequest = await apiClient.fetch(url, options);
  return StudyRequest.read.validateAsync(persistedStudyRequest);
}
