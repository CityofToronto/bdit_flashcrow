...
async function getStudiesByCentreline(feature, studyType, filters) {
  const { centrelineId, centrelineType } = feature;
  const data = {
    centrelineId,
    centrelineType,
  ...
  const options = { data };
  const counts = await apiClient.fetch('/studies/byCentreline', options);
  const countsSchema = Joi.array().items(Study.read);
  return countsSchema.validateAsync(counts);
}
...
async function getStudiesByCentrelineSummary(feature, filters) {
  ...
  const options = { data };
  const summary = await apiClient.fetch('/studies/byCentreline/summary', options);
  const summarySchema = Joi.array().items(
    Joi.object().keys({
