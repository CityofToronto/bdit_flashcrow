...
ReportType.init({
  ...
  WARRANT_TRAFFIC_SIGNAL_CONTROL: {
    ...
    options: {
      adequateTrial: Joi.boolean().required(),
      collisionsTotal: Joi.number().integer().positive().required(),
      preparedBy: Joi.string().required(),
      preventablesByYear: Joi.array().length(3).items(
        Joi.number().integer().positive().required(),
      ).required(),
      startYear: Joi.number().integer().positive().required(),
    },
    tmcRelated: true,
    speedRelated: false,
