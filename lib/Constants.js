...
const SearchKeys = {
  Requests: {
    ...
    CREATED_AT: (q, r) => {
      const qLower = q.toLowerCase();
      const createdAtStr = TimeFormatters.formatDefault(r.createdAt);
    ...
    STUDY_TYPE: (q, r) => {
      const qLower = q.toLowerCase();
      let rLower = r.studyType.label.toLowerCase();
      if (rLower.indexOf(qLower) !== -1) {
        return true;
      }
      rLower = r.studyType.name.toLowerCase();
      return rLower.indexOf(qLower) !== -1;
    },
  },
};
...
const SortKeys = {
  Requests: {
    ...
    CREATED_AT: r => r.createdAt.valueOf(),
    DUE_DATE: r => r.dueDate.valueOf(),
    ID: r => r.id,
    LAST_EDITED_AT:
      r => (r.lastEditedAt === null ? r.createdAt.valueOf() : r.lastEditedAt.valueOf()),
    LOCATION: r => r.location.description,
    REQUESTER: r => r.requestedBy.uniqueName,
    STATUS: r => r.status.ordinal,
    STUDY_TYPE: r => r.studyType.label,
