...
StorageController.push({
  ...
  path: '/mvcr/{filename}',
  options: {
    description: 'Get the MVCR image with the given filename',
    tags: ['api'],
    validate: {
      params: {
  ...
  handler: async (request, h) => {
    ...
    const subdirectory = 'mvcr-temp';
    let fileExists;
    try {
      fileExists = await storageStrategy.has(subdirectory, filename, false);
    } catch (err) {
      const { statusCode } = HttpStatus.BAD_REQUEST;
      return Boom.boomify(err, { statusCode, override: false });
    }
    if (!fileExists) {
      return Boom.notFound(`MVCR not found: ${filename}`);
    }
