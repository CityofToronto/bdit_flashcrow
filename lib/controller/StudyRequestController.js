...
import { cloneStudyRequestAsTransient } from '@/lib/requests/RequestClone';
...
/**
 * Copy the study request with the given ID, creating a new study request with the same
 * transient fields.
 *
 * This endpoint also sends an email to the requester and any persons CC'd.  Note that this
 * email is sent after the bulk study request is created: since email delivery is not 100%
 ...
 *
 * @memberof StudyRequestController
 * @name postStudyRequestCopy
 * @type {Hapi.ServerRoute}
 */
StudyRequestController.push({
  method: 'POST',
  path: '/requests/study/{id}/copy',
  options: {
    auth: {
      scope: [AuthScope.STUDY_REQUESTS.name],
    },
    description: 'Copy the given study request',
    response: {
      schema: StudyRequest.read,
    },
    tags: ['api'],
  ...
  handler: async (request) => {
    ...
    const transientStudyRequestNew = cloneStudyRequestAsTransient(studyRequest);
    const user = request.auth.credentials;
    const persistedStudyRequestNew = await StudyRequestDAO.create(transientStudyRequestNew, user);

    const emailRequestedAdmin = new EmailStudyRequestRequestedAdmin(persistedStudyRequestNew);
    const emailRequested = new EmailStudyRequestRequested(persistedStudyRequestNew);
    await sendEmailsSafe(request, [emailRequestedAdmin, emailRequested]);

    return persistedStudyRequestNew;
  },
});
 ...
 *
 * A request is pending if it is in a status where it is editable.
 *
 * @memberof StudyRequestController
      ...
      schema: Joi.array().items(
        Joi.object().keys({
          ...
          request: Joi.when(
            ...
            {
              is: true,
              then: StudyRequestBulk.read,
              otherwise: StudyRequest.read,
            },
          ),
        }),
      ),
