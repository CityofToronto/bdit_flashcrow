...
import { cloneStudyRequestBulkAsTransient } from '@/lib/requests/RequestClone';
...
/**
 * Copy the bulk study request with the given ID, creating a new bulk study request with the
 * same transient fields.
 *
 * This endpoint also sends an email to the requester and any persons CC'd.  Note that this
 * email is sent after the bulk study request is created: since email delivery is not 100%
 * reliable, and since this depends on AWS SES (which is also not 100% reliable), it is not
 ...
 *
 * @memberof StudyRequestBulkController
 * @name postStudyRequestBulkCopy
 * @type {Hapi.ServerRoute}
 */
StudyRequestBulkController.push({
  method: 'POST',
  path: '/requests/study/bulk/{id}/copy',
  options: {
    auth: {
      scope: [AuthScope.STUDY_REQUESTS.name],
    },
    description: 'Copy the given bulk study request',
    response: {
      schema: StudyRequestBulk.read,
    },
    tags: ['api'],
  ...
  handler: async (request) => {
    ...
    const transientStudyRequestBulkNew = cloneStudyRequestBulkAsTransient(studyRequestBulk);
    const user = request.auth.credentials;
    const persistedStudyRequestBulkNew = await StudyRequestBulkDAO.create(
      transientStudyRequestBulkNew,
      user,
    );

    const emailRequestedAdmin = new EmailStudyRequestBulkRequestedAdmin(
      persistedStudyRequestBulkNew,
    );
    const emailRequested = new EmailStudyRequestBulkRequested(
      persistedStudyRequestBulkNew,
    );
    await sendEmailsSafe(request, [emailRequestedAdmin, emailRequested]);

    return persistedStudyRequestBulkNew;
  },
});
...
StudyRequestBulkController.push({
  ...
  options: {
    auth: {
      scope: [AuthScope.STUDY_REQUESTS_ADMIN.name],
    },
