...
JobController.push({
  ...
  handler: async (request) => {
    ...
    try {
      ...
      if (selectionType.name === 'CORRIDOR' && features.length > MAX_CORRIDOR_POINTS) {
        return Boom.badRequest(`cannot fetch reports on more than ${MAX_CORRIDOR_POINTS} locations`);
      }
      if (selectionType.name === 'POINTS' && features.length > MAX_LOCATIONS) {
...
JobController.push({
  method: 'POST',
  path: '/jobs/compressMvcrs',
  options: {
    auth: {
      scope: [AuthScope.MVCR_READ.name],
    },
    description: 'Add Download All MVCRs job to the Scheduler queue',
    response: {
      schema: JobMetadata.read,
    },
    tags: ['api'],
  ...
  handler: async (request) => {
    ...
    try {
      const jobMetadata = await JobManager.publish(JobType.COMPRESS_MVCRS, data, user);
      return jobMetadata;
    } catch (err) {
      const { statusCode } = HttpStatus.BAD_REQUEST;
      return Boom.boomify(err, { statusCode, override: false });
