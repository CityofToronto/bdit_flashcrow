...
class ReportPedDelaySummary extends ReportBaseFlow {
    ...
    return {
      ...
      header: [
        [
          ...
          { value: 'Percent of Factored Total', style: { br: true } },
          { value: `Delay ${PedDelay.DELAYED.description}` },
          { value: 'Percent of Factored Total', style: { br: true } },
          { value: 'Factored Total' },
        ],
      ],
      body: [
        ...stats.zones.map(
  ...
  static getCountMetadataOptions({
    ...
    const entries = [
      ...
      ...zones.map(zone => ({
        cols: 3,
        name: `Zone ${zone.label} Factored Total`,
        value: zone.totals.total_f,
      })),
      { cols: 3, name: 'All Zones Factored Total', value: totals.total_f },
      { cols: 12, name: 'Notes', value: notes },
    ];
    return { entries };
  }
  ...
  generateLayoutContent(study, reportData) {
    const infoOptions = {
      text:
        'Note: Factored Totals and Factored Summary are factored by 2 for assisted Pedestrian Types ("Assisted Children", "Unassisted Children", "Senior Citizens", and "Persons with Disabilities")',
    };
