...
let specialCaseCentrelineIds = [];
SpecialCasesDAO.getSpecialCaseCentrelineIds().then((ids) => {
  specialCaseCentrelineIds = ids;
});
...
class ReportBaseFlowDirectional extends ReportBaseFlow {
  ...
  static inferRoadDirections(roads, directionCandidates) {
    ...
    const mergedRoadDirections = [];
    const merged = new Set();

    roadDirections.forEach((road, i) => {
      if (merged.has(i)) return;
      const opposingRoadIndex = roadDirections.findIndex((otherRoad, j) => j !== i
        && !merged.has(j)
        && road.some(dir => otherRoad.some(otherDir => otherDir === dir.opposing)));
      if (opposingRoadIndex !== -1) {
        mergedRoadDirections.push([...road, ...roadDirections[opposingRoadIndex]]);
        merged.add(i);
        merged.add(opposingRoadIndex);
      } else {
        mergedRoadDirections.push(road);
        merged.add(i);
      }
    });
    return mergedRoadDirections;
  }
  ...
  static getDirectionalStats(study, { countData: rawData, intersection, segments }) {
     ...
     *
     * UPDATE!
     *  March 17, 2026: We are moving away from the process defined above
     * After a conversation with traffic ops, it was decided that having ONE single major road
     * made the most sense. The logic below has been adjusted accordingly.
     */
    ...
    const totalData = ArrayUtils.sumObjects(hourlyData);
    const totalRoadVolumes = roadDirections.map(
      directions => ReportBaseFlowDirectional.getRoadVolume(directions, totalData),
    );
    const totalMajorIndex = ArrayUtils.getMaxIndexBy(totalRoadVolumes, identity);
    const majorDirections = roadDirections[totalMajorIndex];
    const minorDirections = Array.prototype.concat.apply(
      [],
      roadDirections.filter((_, i) => i !== totalMajorIndex),
    );
    const hourlyMajorDirections = hourlyData.map(() => majorDirections);
    const hourlyMinorDirections = hourlyData.map(() => minorDirections);
    return {
      countData,
      hourlyData,
