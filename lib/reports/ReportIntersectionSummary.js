...
class ReportIntersectionSummary extends ReportBaseFlowDirectional {
  ...
  static computeHourlyTotalsMajorOnly(tmcData, majorDirections) {
    const data = {};
    ...
    data.MAJOR_CROSSING_TOTAL = crossingTotalStep3
      + crossingTotalStep4;
     ...
     */
    data.TOTAL = data.MAJOR_APPROACHES;
  ...
  generateCsv(count, { hourlyTotals, timeRanges }) {
    ...
    const dataColumns = dataKeys.map(key => ({
      ...
      header: (['_BIKE', '_OTHER'].some(element => key.includes(element)) ? `${key.slice(0, 1)}A${key.slice(1)}`
        : ['_L', 'N_T', '_R', 'E_T', 'S_T', 'W_T'].some(element => key.includes(element)) ? `${key.slice(0, 1)}B${key.slice(1)}` : key),
    }));
    const columns = [
