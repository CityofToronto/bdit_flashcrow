...
class ReportCountSummaryTurningMovementDetailed extends ReportBaseFlow {
  ...
  transformData(countData) {
    const totaledData = ReportBaseFlowDirectional.computeAllMovementAndVehicleTotals(countData);
    return totaledData.map(({
      id,
      countId,
      t: end,
      data,
      ...
      const t = new Date(
        ...
        end.getMinutes() - ReportBaseFlow.MINUTES_PER_ROW,
        end.getSeconds(),
      );
      return {
        id,
