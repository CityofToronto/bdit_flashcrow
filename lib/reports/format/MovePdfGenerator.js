...
class MovePdfGenerator {
  constructor({
    ...
    this.width = width;
    this.height = height;
    this.margin = FormatCss.var('--space-m');
    ...
    if (this.reportType.label === 'Collision Directory Report') {
      this.filters = this.formatFilters(this.generateFilters(options));
    }
  }
          ...
          columns: [
            ...
            },
            {
              ...
              stack: [
                ...
                {
                  text: (this.reportType.label === 'Collision Directory Report' ? this.generateDateRange(this.options) : this.header.subinfo),
                  alignment: 'right',
                  color: colorPrimaryDark,
                  fontSize: fontSizeXS,
                },
  ...
  generateFilters(filters) {
    ...
    if (drivact.length > 0) {
      const collisionFilter = getCollisionFilterChip('drivact', drivact, this.collisionFactors);
      collisionFilters.push(collisionFilter);
    }
    if (drivcond.length > 0) {
      const collisionFilter = getCollisionFilterChip('drivcond', drivcond, this.collisionFactors);
      collisionFilters.push(collisionFilter);
    }
    if (impactype.length > 0) {
      const collisionFilter = getCollisionFilterChip('impactype', impactype, this.collisionFactors);
      collisionFilters.push(collisionFilter);
    }
    if (initdir.length > 0) {
      const collisionFilter = getCollisionFilterChip('initdir', initdir, this.collisionFactors);
      collisionFilters.push(collisionFilter);
    }
    if (injury.length > 0) {
      const collisionFilter = getCollisionFilterChip('injury', injury, this.collisionFactors);
      collisionFilters.push(collisionFilter);
    }
    if (manoeuver.length > 0) {
      const collisionFilter = getCollisionFilterChip('manoeuver', manoeuver, this.collisionFactors);
      collisionFilters.push(collisionFilter);
    }
    if (rdsfcond.length > 0) {
      const collisionFilter = getCollisionFilterChip('rdsfcond', rdsfcond, this.collisionFactors);
      collisionFilters.push(collisionFilter);
    }
    if (vehtype.length > 0) {
      const collisionFilter = getCollisionFilterChip('vehtype', vehtype, this.collisionFactors);
      collisionFilters.push(collisionFilter);
    }
  ...
  formatFilters(filters) {
    ...
    const formattedFilters = filters.map(item => ({
      text: [
        { text: `${filtersMap[item.filter]}: `, bold: true, fontSize: 12 },
