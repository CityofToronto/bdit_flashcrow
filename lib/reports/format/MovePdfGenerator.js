...
class MovePdfGenerator {
  ...
  static getMetadataBlock({ name, value }) {
    if (value === null) {
  ...
  /* eslint-disable class-methods-use-this */
  generateCountMetadata({ left, right }) {
    const blocksLeft = left.map(MovePdfGenerator.getMetadataBlock);
    const blocksRight = right.map(MovePdfGenerator.getMetadataBlock);
  ...
  static normalizeCell(cell, header) {
    ...
    const style = MovePdfGenerator.normalizeStyle(cellStyle, cellHeader);
    return {
      value,
      rowspan,
      colspan,
    ...
    section.forEach((row, r) => {
      ...
      row.forEach((cell) => {
        ...
        if (fontSizeStr !== null) {
          ...
          const size = FormatCss.var(`--font-size-${MovePdfGenerator.DEFAULT_FONT_SIZE}`);
  ...
  generateContent() {
    return Array.prototype.concat.apply(
      ...
      this.content.map(({ type: blockType, options }) => {
        ...
        contentBlocks[n - 1].margin = [0, 0, 0, this.margin];
  ...
  generateFooter(currentPage, pageCount) {
    ...
    const dateStr = TimeFormatters.formatDateTime(this.reportDate);
  ...
  generateDocDefinition() {
    const fontSize = FormatCss.var(`--font-size-${MovePdfGenerator.DEFAULT_FONT_SIZE}`);
    const spaceM = FormatCss.var('--space-m');
