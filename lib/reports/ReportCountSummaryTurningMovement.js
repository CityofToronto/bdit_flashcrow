...
class ReportCountSummaryTurningMovement extends ReportBaseFlow {
  ...
  static getIntervalsWithinPeriod(intervals, period) {
    ...
    if (startIndex !== -1) {
      ...
      if (endIndex === -1) {
        intervalsInPeriod = postStartIntervals;
      } else {
  ...
  static totalsForPeak(intervalTotals, window) {
    let peakSummary = this.statSummaryForNullCount();
    ...
    if (intervalsAtPeak !== null) {
      let period = null;
      if (intervalsAtPeak.length < 4) {
        period = {
          startTime: intervalsAtPeak[0].t,
          endTime: intervalsAtPeak[0].t.plus(this.PEAK_DURATION),
        };
      }
      peakSummary = this.totalsForPeriod(intervalsAtPeak, period);
    }
    return peakSummary;
  }
