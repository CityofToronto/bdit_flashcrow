...
class ReportCountSummaryTurningMovement extends ReportBaseFlow {
  ...
  static totalsForPeak(intervalTotals, window) {
    ...
    const peakSummary = this.statSummaryForNullCount();

    if (intervalsAtPeak !== null) {
      peakSummary.sum = ArrayUtils.sumObjects(intervalsAtPeak.map(interval => interval.data));
      peakSummary.timeRange = {
  ...
  static totalsForPeriod(intervalTotals, period) {
    const intervalsInPeriod = this.getIntervalsWithinPeriod(intervalTotals, period);
    const sum = ArrayUtils.sumObjects(intervalsInPeriod.map(interval => interval.data));
  ...
  static transformCountData(countData, studyDate) {
    ...
    if (countData.length > 0) {
      ...
      const nTotalIntervals = countData.length;
      const studyPeriod = {
        startTime: countData[0].t,
        endTime: countData[nTotalIntervals - 1].t.plus(this.COUNT_INTERVAL_DURATION),
      };
      statSummaries.all = this.totalsForPeriod(totaledData, studyPeriod);
    }
