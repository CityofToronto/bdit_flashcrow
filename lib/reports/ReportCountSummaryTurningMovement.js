...
class ReportCountSummaryTurningMovement extends ReportBaseFlow {
  ...
  static getCandidatesForPeak(countIntervals,
    peakWindow, twoHour = false) {
    let peakDuration;
    if (twoHour) {
      peakDuration = REPORT_CONSTANTS.TWO_HOUR_PEAK_DURATION;
    } else peakDuration = REPORT_CONSTANTS.PEAK_DURATION;
    const candidates = [];
  ...
  static totalsForPeak(intervalTotals, peakWindow, twoHour = false) {
    let peakTotalsSummary = this.statSummaryForNullCount();
    let peakCandidates;
    if (twoHour) {
      peakCandidates = this.getCandidatesForPeak(intervalTotals, peakWindow, true);
    } else peakCandidates = this.getCandidatesForPeak(intervalTotals, peakWindow);
  ...
  static transformCountData(countData) {
    ...
    if (countData.length > 0) {
      ...
      statSummaries.am = this.totalsForPeak(totaledData, am2HourPeriod, true);

      const pm2HourPeriod = this.dateTimePeriodFromObject(
        REPORT_CONSTANTS.PM_2_HOUR_PERIOD, studyDate,
      );
      statSummaries.pm = this.totalsForPeak(totaledData, pm2HourPeriod, true);

      const offHoursPeriod = this.dateTimePeriodFromObject(
        {
          startTime: {
            hour: statSummaries.amPeak.timeRange.end.c.hour,
            minute: statSummaries.amPeak.timeRange.end.c.minute,
          },
          endTime: {
            hour: statSummaries.pmPeak.timeRange.start.c.hour,
            minute: statSummaries.pmPeak.timeRange.start.c.minute,
          },
        }, studyDate,
      );
  ...
  createCsvHeader(key) {
    if (['PEDS', 'BIKE', 'OTHER'].some(element => key.includes(element))) {
      return `${key.slice(0, 1)}A${key.slice(1)}`;
    }
    if (['BUS', 'TRUCK', 'CAR'].some(element => key.includes(element))) {
      return `${key.slice(0, 1)}B${key.slice(1)}`;
    }
    return key;
  }
  ...
  generateCsv(count, { stats }) {
    ...
    const dataColumns = dataKeys.map(key => ({
      ...
      header: (['_PEDS', '_BIKE', '_OTHER'].some(element => key.includes(element)) ? `${key.slice(0, 1)}A${key.slice(1)}`
        : ['BUS', 'TRUCK', 'CAR', '_VEHICLE'].some(element => key.includes(element)) ? `${key.slice(0, 1)}B${key.slice(1)}` : key),
    }));
    const columns = [
