...
class ReportCountSummaryTurningMovement extends ReportBaseFlow {
  ...
  static totalsForPeriod(intervalTotals, period) {
    ...
    if (intervalTotals.length > 0) {
      ...
      timeRange = {
        ...
        nIntervals: intervalsInPeriod.length,
      };
    }
  ...
  static transformCountData(countData, studyDate) {
    ...
    const offHoursCountsSum = this.totalsForPeriod(countData, offHoursPeriod);
    const offHoursAverages = this.averagesForPeriod(offHoursCountsSum);
    offHoursAverages.avg = RBFD.computeMovementAndVehicleTotals(offHoursAverages.avg);
