...
class ReportCountSummaryTurningMovement extends ReportBaseFlow {
  ...
  static filterAmPeakWindowCounts(counts, date) {
    const startTimeObj = { ...date.toObject(), ...{ hour: 7, minute: 30 } };
    const startTime = DateTime.fromObject(startTimeObj);

    const endTimeObj = { ...date.toObject(), ...{ hour: 9, minute: 30 } };
    const endTime = DateTime.fromObject(endTimeObj);

    const windowIndices = mapPeriodToIntervalIndices(counts.map(d => d.t), startTime, endTime);
    const filteredCounts = counts.slice(windowIndices.start, windowIndices.end + 1);
    return filteredCounts;
  }

  static filterPeakPeriodCounts(counts) {
    const allPeriodTotals = [];

    counts.forEach((periodStartCount, periodStartIndex) => {
      const periodEndTime = periodStartCount.t.plus({ hours: 1 });
      let periodEndIndex;
      for (let n = 0; n < counts.length; n++) {
        const nIntervalEndTime = counts[n].t.plus({ minutes: 15 });
      ...
      if (periodEndIndex) {
        const periodCounts = counts.slice(periodStartIndex, periodEndIndex + 1);
        allPeriodTotals.push({
          startIndex: periodStartIndex,
          endIndex: periodEndIndex,
          startIntervalTime: periodStartCount.t,
          endIntervalTime: counts[periodEndIndex].t,
          totalVehicles: periodCounts.reduce((p, n) => p + n.data.VEHICLE_TOTAL, 0),
        });
      }
    });

    let peakPeriodCounts = null;
    if (allPeriodTotals.length > 0) {
      const allPeriodsSortedByTotal = allPeriodTotals.sort(
        (periodA, periodB) => periodB.totalVehicles - periodA.totalVehicles,
      );
      const peakPeriod = allPeriodsSortedByTotal[0];
      peakPeriodCounts = counts.slice(peakPeriod.startIndex, peakPeriod.endIndex + 1);
    }
    return peakPeriodCounts;
  }

  static amPeak(intervalTotals, date) {
    let amPeakSums;
    let amPeakTimeRange;
    if (intervalTotals.length > 0) {
      const amPeakWindowCounts = this.filterAmPeakWindowCounts(intervalTotals, date);
      const amPeakCounts = this.filterPeakPeriodCounts(amPeakWindowCounts);
      amPeakSums = ArrayUtils.sumObjects(amPeakCounts.map(c => c.data));
      amPeakTimeRange = {
        start: amPeakCounts[0].t,
        end: amPeakCounts[amPeakCounts.length - 1].t.plus({ minutes: 15 }),
      };
    } else {
