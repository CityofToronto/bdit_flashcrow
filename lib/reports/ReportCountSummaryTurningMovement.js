...
class ReportCountSummaryTurningMovement extends ReportBaseFlow {
  ...
  static getIntervalsWithinPeriod(intervals, date, period = this.AM_PEAK_WINDOW) {
    const startTimeObj = { ...date.toObject(), ...period.startTime };
    ...
    const endTimeObj = { ...date.toObject(), ...period.endTime };
    const endTime = DateTime.fromObject(endTimeObj);
    const endIndex = this.findLastIntervalIndexForPeriod(postStartIntervals, endTime);
  ...
  static getPeakPeriodForWindowOfTime(intervalTotals, date, window = this.AM_PEAK_WINDOW) {
    let peakSums;
    let peakTimeRange;
    const intervalsInWindow = this.getIntervalsWithinPeriod(intervalTotals, date, window);
    const intervalsAtPeak = this.getIntervalsAtPeak(intervalsInWindow);
    if (intervalTotals.length > 0 && intervalsAtPeak !== null) {
      peakSums = ArrayUtils.sumObjects(intervalsAtPeak.map(c => c.data));
      peakTimeRange = {
      ...
      peakSums = ReportBaseFlowDirectional.computeMovementAndVehicleTotals(emptyTMC);
      peakTimeRange = null;
    }
  ...
  static transformCountData(countData, date) {
    ...
    const amPeak = this.getPeakPeriodForWindowOfTime(totaledData, date);
    const pmPeak = this.getPeakPeriodForWindowOfTime(totaledData, date, this.PM_PEAK_WINDOW);
