...
class ReportWarrantTrafficSignalControl extends ReportBaseFlowDirectional {
  ...
  transformData(study, {
    ...
    const threshold3A = ReportWarrantTrafficSignalControl.getThresholds3A(minVolumeMajorOnly.a);
      ...
      body: [
        ...
        [
          ...
          },
          {
            ...
            potentially reduceable collisions
            meets at least one benchmark`,
            header: true,
            style: { bt: true },
          },
          {
  ...
  static getPreventableCollisionsTableOptions(sectionData, minVolumes) {
    ...
    const threshold3AMet = Math.max(
      ...(sectionData.severityCompliance),
      ...(sectionData.ksiCompliance),
    );
    const volume = ReportWarrantTrafficSignalControl.VOLUME_FACTOR
    * minVolumes.hourly.reduce((a, c) => a + c.value, 0);
    ...
    return {
      ...
      body: [
        ...
        [
          {
            value: `(based on AADT
            ${thresholdString})`,
            style: { br: true, 'text-center': true },
          },
          { value: null },
        ...
        [
          {
            value: `(any one value must meet or
        ...
        [
          { value: 'Actual %', style: { fontWeight: 'bold', br: true }, header: true },
          { value: `${sectionData.severityCompliance[0]}%` },
          { value: `${sectionData.ksiCompliance[0]}%`, style: { br: true } },
          { value: `${sectionData.severityCompliance[1]}%` },
          { value: `${sectionData.ksiCompliance[1]}%`, style: { br: true } },
          { value: `${sectionData.severityCompliance[2]}%` },
          { value: `${sectionData.ksiCompliance[2]}%`, style: { br: true } },
          { value: `${sectionData.severityCompliance[3]}%` },
          { value: `${sectionData.ksiCompliance[3]}%` },
        ],
      ],
      footer: [
        [
          ...
          {
            value: threshold3AMet,
            style: { bold: true, fontSize: 'l', bt: true },
          },
        ],
      ],
    };
...
ReportWarrantTrafficSignalControl.THRESHOLD_STRING_I = 'less than 5,000';
ReportWarrantTrafficSignalControl.THRESHOLD_STRING_II = 'between 5,000 and 8,000';
ReportWarrantTrafficSignalControl.THRESHOLD_STRING_III = 'between 8,000 and 15,000';
ReportWarrantTrafficSignalControl.THRESHOLD_STRING_IV = 'greater than 15,000';

/*
 * This was calculated by Nate and David
 * What we are attempting here is to determine
 * a conversion from 24H daily volume to
 * 8H thresholds.
 * The factor was arrived at by running
 * against a large set of mivision data, all motor vehicle types
 * restricted to Tues-Thurs
 */
ReportWarrantTrafficSignalControl.VOLUME_FACTOR = 2.1;
