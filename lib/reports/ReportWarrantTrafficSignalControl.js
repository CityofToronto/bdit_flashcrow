...
class ReportWarrantTrafficSignalControl extends ReportBaseFlowDirectional {
  ...
  static getThresholds(minFeatureCode, segments) {
    ...
    return {
      ...
      threshold1B,
      threshold2A,
      threshold2B: ReportWarrantTrafficSignalControl.THRESHOLD_2B,
      threshold3A: ReportWarrantTrafficSignalControl.THRESHOLD_3A,
    };
  }

  static getCompliance(value, threshold) {
    const complianceFactor = Math.min(1, value / threshold);
    return Math.round(ReportWarrantTrafficSignalControl.COMPLIANCE_FULL * complianceFactor);
  }

  static getOverallCompliance(hourlyCompliance) {
    const totalCompliance = ArrayStats.sum(
      hourlyCompliance.map(({ compliance }) => compliance),
    );
    return Math.round(totalCompliance / hourlyCompliance.length);
  }

  static evaluateMinVolumeWarrant(hourlyTotals, threshold1A, threshold1B) {
    const hourly1A = hourlyTotals.map(({ TOTAL: value }) => {
      const compliance = ReportWarrantTrafficSignalControl.getCompliance(value, threshold1A);
      return { value, compliance };
    });
    const compliance1A = ReportWarrantTrafficSignalControl.getOverallCompliance(hourly1A);

    const hourly1B = hourlyTotals.map(({ MINOR_APPROACHES: value }) => {
      const compliance = ReportWarrantTrafficSignalControl.getCompliance(value, threshold1B);
      return { value, compliance };
    });
    const compliance1B = ReportWarrantTrafficSignalControl.getOverallCompliance(hourly1B);

    const compliance = Math.min(compliance1A, compliance1B);
  ...
  static evaluateDelayToCrossWarrant(hourlyTotals, threshold2A, threshold2B) {
    const hourly2A = hourlyTotals.map(({ MAJOR_APPROACHES: value }) => {
      const compliance = ReportWarrantTrafficSignalControl.getCompliance(value, threshold2A);
      return { value, compliance };
    });
    const compliance2A = ReportWarrantTrafficSignalControl.getOverallCompliance(hourly2A);

    const hourly2B = hourlyTotals.map(({ MAJOR_CROSSING_TOTAL: value }) => {
      const compliance = ReportWarrantTrafficSignalControl.getCompliance(value, threshold2B);
      return { value, compliance };
    });
    const compliance2B = ReportWarrantTrafficSignalControl.getOverallCompliance(hourly2B);

    const compliance = Math.min(compliance2A, compliance2B);
  ...
  static evaluateCollisionHazardWarrant(options, threshold3A, minVolume, delayToCross) {
    const { adequateTrial: met3B, preventablesByYear: annual3A } = options;

    const value3A = ArrayStats.sum(annual3A) / annual3A.length;
    const compliance3A = ReportWarrantTrafficSignalControl.getCompliance(value3A, threshold3A);

    const value3B = met3B ? 1 : 0;
    const compliance3B = ReportWarrantTrafficSignalControl.getCompliance(value3B, 1);
    ...
    const met3C = met1 || met2;
    const value3C = met3C ? 1 : 0;
    const compliance3C = ReportWarrantTrafficSignalControl.getCompliance(value3C, 1);

    const compliance = Math.min(compliance3A, compliance3B, compliance3C);
    ...
    return {
      ...
      b: {
        compliance: compliance3B,
        met: met3B,
        threshold: 1,
        value: value3B,
      },
      c: {
        compliance: compliance3C,
        met: met3C,
        threshold: 1,
        value: value3C,
      },
  ...
  static evaluateCombinationWarrant(minVolume, delayToCross) {
    ...
    const met = met1 && met2;
    const value = met ? 1 : 0;
    const compliance = ReportWarrantTrafficSignalControl.getCompliance(value, 1);
    return {
      compliance,
      met,
      threshold: 1,
