...
class ReportWarrantTrafficSignalControl extends ReportBaseFlowDirectional {
  ...
  static getCompliance(value, threshold) {
    ...
    const compliance = ReportWarrantTrafficSignalControl.COMPLIANCE_FULL * value / threshold;
    if (compliance >= ReportWarrantTrafficSignalControl.COMPLIANCE_PARTIAL) {
  ...
  static evaluateCollisionHazardWarrant(options, threshold3A, minVolume, delayToCross) {
    const { adequateTrial: compliance3B, preventablesByYear: annual3A } = options;
