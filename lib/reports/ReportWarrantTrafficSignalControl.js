...
class ReportWarrantTrafficSignalControl extends ReportBaseFlowDirectional {
  ...
  static getSummaryTableOptions(reportData) {
    const threshold3AMet = Math.max(
      ...(reportData.collisionHazard.a.severityCompliance.slice(0, 3)),
    );
    return {
  ...
  static calculateThresholds = (volume) => {
    ...
    if (volume < ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_LOW) {
      ...
      thresholdValue = ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_LOW;
      ...
      thresholdValue = ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_MID;
      ...
      thresholdValue = ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_HIGH;
    }
    return {
      annualSeverityFactor,
      annualKsiFactor,
  ...
  static getPreventableCollisionsTableOptions(sectionData, minVolumes) {
    ...
    const dateString = thresholdValue === ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_LOW
      ? 'less than 2500'
      // eslint-disable-next-line no-nested-ternary
      : thresholdValue === ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_MID
        ? 'between 2500 and 5000'
        : thresholdValue === ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_HIGH
          ? 'greater than 5000' : null;
    return {
      ...
      body: [
        ...
        [
          { value: `Minimum Required (Based on 8H volume ${dateString})` },
          { value: annualSeverityFactor },
          { value: annualKsiFactor },
          { value: annualSeverityFactor },
          { value: annualKsiFactor },
          { value: annualSeverityFactor },
          { value: annualKsiFactor },
  ...
  generateLayoutContent(count, reportData) {
    ...
    const section3ATableOptions = ReportWarrantTrafficSignalControl
      .getPreventableCollisionsTableOptions(reportData.collisionHazard.a, reportData.minVolume.a);
 ...
 * @type {number}
 */
ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_LOW = 2500;

/**
 * @type {number}
 */
ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_MID = 4000;

/**
 * @type {number}
 */
ReportWarrantTrafficSignalControl.ADTV_BENCHMARK_HIGH = 7500;
